...
To Deal Damage to [[Unit|Units]], mark the specified amount of Damage on the [[Unit]]. Damage is marked on each unit separately.
Damage can be Dealt to more than one Unit at the same time.

**Valid Damage** is a positive integer amount, greater than or equal to 1 Damage. **Only Valid Damage is Dealt.**
...
Only Damage can be Dealt. Assigning Damage causes Damage to be dealt outside of being directed to *Deal Damage*.

Dealing can have the intrinsic property of [[Bonus Damage]]. Bonus Damage is a property granted to the action of Dealing and alters the amount of Damage distributed by this action.

Deal actions can originate from one or more sources. If a game effect does not specify a source, the game effect describing the Deal action is the source.
...
[[Unit|Units]] and [[Spell|Spells]] can be the source of Damage for Deal actions.
[[Ability|Abilities]] can be the source of Damage for Deal actions. When an Ability is the source of Damage for a Deal action, it is in addition to the Spell or Unit that created that Ability.
...
Damage Dealt as a result of being assigned during [[Combat]] has the [[Unit|Units]] as its source. The Damage assigned, and subsequently Dealt, to attackers has the defenders as the source and vice versa.
